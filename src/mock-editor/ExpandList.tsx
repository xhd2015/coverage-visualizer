import {
  ...
  export function ExpandListItemRender(props: ExpandListItemRenderProps) {
    ...
    let leafNode
    if (true) {
      leafNode = expanded !== false ? folderOpen : folder
    } else {
      leafNode = (props) => React.createElement(expanded !== false ? VsFolderOpen : VsFolder, {
        width: 12,
        height: 12,
        ...props,
        style: {
    ...
    if (props.expandIconUseV1) {
      leafNode = expanded !== false ? (props: { className?: string, style?: CSSProperties }) => < BsChevronDown
        style={props?.style}
    ...
    return (
      <div
        onClick={noExpandOnClick ? undefined : doClick}
        style={{
          display: 'flex',
          alignItems: 'center',
          paddingLeft: '0.6em',
